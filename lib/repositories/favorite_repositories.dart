...
class FavoriteRepository{
  ...
  Future<List<QueryDocumentSnapshot<FavoriteModel>>> getFavorites(String recipeId, String userId) async {
    try {
      var data = await favoriteRef.where("user_id", isEqualTo: userId).where("recipe_id", isEqualTo: recipeId).get();
  ...
  Future<bool> favorite(FavoriteModel? isFavorite, String recipeId, String userId)  async{
    try {
      if(isFavorite==null){
        await favoriteRef.add(FavoriteModel(userId: userId, recipeId: recipeId));
      }else{
